...
function withOpLock(fn) {
    ...
    const guard = Promise.race([
        ...
        new Promise((_, reject) => {
            timer = setTimeout(() => reject(new Error(`操作锁超时（${OP_LOCK_TIMEOUT_MS / 1000}s 未落定）——本次操作疑似 await 某个永不 settle 的 Promise`
                + '（典型：重载目标 fiber 的 apply() 挂住 → dispose() 永不返回）。已放行后续操作，本次操作被放弃。')), OP_LOCK_TIMEOUT_MS);
            timer.unref?.();
        }),
    ]);
...
export function apply(ctx, config) {
    ...
    function hasActiveEntry(pkgName) {
        ...
        for (const entry of ctx.loader.entries()) {
            ...
            if (nameOk && entry.fiber && FIBER_NAMES[entry.fiber.state] === 'active')
                return true;
        }
        return false;
    }
    ...
    async function reloadPackage(match, urlMatch) {
                ...
                for (const entry of ctx.loader.entries()) {
                    ...
                    if (opts?.name && String(opts.name).includes(match)) {
                        ...
                        if (fiber && typeof fiber === 'object') {
                            ...
                            await disposeFiberSafely(fiber, match, () => { });
        ...
        try {
            ...
            const settled = await awaitFibersSettled(newFibers, FIBER_DISPOSE_TIMEOUT_MS, newFibers.map((_f, i) => `${match}#${i}`));
            if (settled.unsettled.length)
                abandoned.push(...settled.unsettled);
        }
        catch (e) {
            // 整体失败：回滚缓存 + 用旧插件重建
            for (const [u, job] of backup)
    ...
    async function uninject(match, allowSelf = false) {
        ...
        if (match.includes('super-injector') && !allowSelf)
            return 'ERROR: 拒绝卸载 dsh-super-injector 自身（引导器不可卸载；自举卸载需 self=true）';
        const steps = [];
        let fullName = null;
    ...
    function clientSkeletonProblems(base) {
        ...
        try {
            ...
            if (existsSync(libClient)) {
                ...
                try {
                    const pkgJson = JSON.parse(readFileSync(join(base, 'package.json'), 'utf8'));
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            ...
            if (!names.length)
                return 'ERROR: 未找到任何预设（' + presetsRoot + '）';
            const out = [];
            for (const name of names) {
        ...
        async execute(args) {
            return withOpLock(async () => {
                ...
                try {
                    ...
                    // ⚠️ 2026-09-13 事故修正：dsh.profile.bundles 是「自带 dsh.bundle.patch
                    // 的补丁层」专用——无该声明的包写进 bundles 会让 dsh-app-boot 启动核对
                    // 失败（declares no dsh.bundle.patch）、systemd 每 3 秒重启循环（实测）。
                    // 普通插件走 dependencies + cordis.patch.yml 的 insert 行（skill-switches 同路）。
                    const hasBundlePatch = Boolean(pkg?.dsh?.bundle?.patch);
                    if (hasBundlePatch) {
                        ...
                        if (profilePkg.dsh.profile.bundles.includes(name)) {
                            // 幂等纠偏：旧版本误写入的 bundles 行本次顺手摘除
                            profilePkg.dsh.profile.bundles = profilePkg.dsh.profile.bundles.filter((b) => b !== name);
                            steps.push('bundles 摘除误写行（包无 dsh.bundle.patch）');
                        }
                        const idShort2 = name.split('/').pop();
                        if (writePatch('\n# ' + name + '（dev_install_package：无 dsh.bundle.patch → 普通 loader entry，勿写入 bundles）\n- insert:\n    - id: ' + idShort2 + "\n      name: '" + name + "'\n"))
                            steps.push('patch insert 行已写（普通 entry 路径）');
                        else
                            steps.push('patch insert 已存在（幂等跳过）');
                    }
                    ...
                    return 'OK: ' + name + ' 热装配完成\n- ' + steps.join('\n- ') + `\n- ${client}\n（重启装配路径：有 dsh.bundle.patch → bundles 补丁层；无 → patch insert 普通 entry。2026-09-13 起安装器自动分流，不再无条件写 bundles）`;
                }
                catch (e) {
